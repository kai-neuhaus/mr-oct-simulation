...
format_plot()
# manuscript:lst:mronoattnvstime
...
w = 2*pi*f_D
I_t = []
def G(): return exp(-(tau_scan-t0-(N-1)*D_T)**2 / (2*(T_sigma/N)**2)) # envelope
def O(): return exp(-1j * N * w * tau_scan)  # carrier
def T_PM(): return (1-R_PM)**2 * R_PM**(N-1) # attenuation PM splitting ratio
